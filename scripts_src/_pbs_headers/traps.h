...
#define gain_exp_for_trapkill(exp)			give_exp_points(exp); \
											display_msg(sprintf(message_str(SCRIPT_TEST2, 2001), exp));
...
procedure trap_setoff_melee(variable critter, variable dmgMin, variable dmgMax, variable dmgType, variable trapFlags);
...
procedure trap_setoff_effect(variable armPid, variable trapObj, variable trapType, variable trapFlags, variable critter) begin
  if (armPid) then begin
    ...
    call trap_setoff_melee(critter, dmgMin, dmgMax, DMG_normal_dam, trapFlags);
  end
end
...
procedure trap_damage_critter(variable critter, variable dmgMin, variable dmgMax, variable dmgType, variable criticalBodyPart, variable stop) begin
  ...
  if (not pbs_trap_last_target_dead) then begin
    ...
    // TODO: stop for several turns
    // if (stop) then begin end
...
procedure trap_setoff_explosion(variable trapObj, variable mainTarget, variable dmgMin, variable dmgMax, variable radius, variable dmgType, variable trapFlags, variable effectPid, variable sfx) begin
  ...
  foreach (obj in extraTargets) begin
    if (not is_critter_dead(obj) and obj != mainTarget) then begin
      ...
      /*if (get_critter_stat(obj, STAT_current_hp) <= get_expected_damage(obj, dmg, dmgType)) then begin
         if (not combat_is_initialized and not pbs_trap_victims[obj]) then begin
      	   totalExp += exp_for_kill_critter_pid(obj_pid(obj));
...
*/
procedure trap_setoff_melee(variable critter, variable dmgMin, variable dmgMax, variable dmgType, variable trapFlags) begin
  /*expectedDmg := get_expected_damage(critter, dmg, dmgType);
  if (critter_state(critter) == CRITTER_IS_NORMAL and get_critter_stat(critter, STAT_current_hp) <= expectedDmg) then begin
    //display_array(pbs_trap_victims);
    //n := pbs_trap_victims[critter];
    if (not combat_is_initialized and not pbs_trap_victims[critter]) then begin
       gain_exp_for_trapkill(exp_for_kill_critter_pid(obj_pid(critter)))
       mod_kill_counter(critter_kill_type(critter), 1);
    end
    pbs_trap_victims[critter] := true;
  end else begin
    if (pbs_ini_trap_reveals_dude) then begin
      call add_array_set(load_create_array(ARR_ANGRY_TEAMS, 0), obj_team(critter));
    end
  end*/
  pbs_trap_last_target := critter;
  variable
    criticalBodyPart := (BODY_HIT_RIGHT_LEG + random(0, 1)) if (trapFlags bwand TRAP_FLAG_CRITICAL) else -1,
    killExp := trap_damage_critter(critter, dmgMin, dmgMax, dmgType, criticalBodyPart, 1);
