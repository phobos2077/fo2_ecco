...
procedure Damage_Door(variable dmg);
...
procedure damage_p_proc begin
   ...
   */
   if (DOOR_STATUS == STATE_METAL) then
      DmgTresh := METAL_TRESHOLD;
   attacker := source_obj;
   weapon := obj_being_used_with;
   if not(is_success(pbs_last_hit_roll_result)) then 
      return;
   dmg := fixed_param ;
   if (dmg > 0) then begin  // this only happens when blowing the door with dynamite
      if (attacker == 0 and weapon == 0) then 
         dmg := random(30, 50);
   end else begin
      if (attacker) then begin
         if (weapon == 0 or weapon == self_obj) then begin // HTH attack
            ...
            dmg := get_proto_data(obj_pid(weapon), 44); // max dmg
            if (pbs_last_burst_size > 1) then
               dmg *= pbs_last_burst_size;
            dmg := random(get_proto_data(obj_pid(weapon), 40), dmg);
            display_msg("dmg="+dmg);
         end
      end
   end
...
/* Set up the door state when the player first enters the map */
   if (local_var(LVAR_Set_Door_Status) == 0) then begin
       ...
       set_local_var(LVAR_Door_HP,DOOR_HEALTH);
   end else if (local_var(LVAR_Door_HP) == 0) then begin
       set_local_var(LVAR_Door_HP,DOOR_HEALTH);
   end
...
/* The trap is now disarmed and should never go off again.                           */
   set_local_var(LVAR_Trapped, STATE_INACTIVE);
   /* also damage the door iteself */
   call Damage_Door(Trap_Damage);
end
...
procedure Damage_Door(variable dmg) begin
   if (WATCHER_OBJ != 0) then begin
      ...
      //call add_array_set(load_create_array(ARR_ANGRY_TEAMS, 0), obj_team(WATCHER_OBJ));
      attack_setup(WATCHER_OBJ, dude_obj);
   end
   if (DOOR_STATUS != STATE_NON_DESTROY) then begin
      dec_local_var_amt(LVAR_Door_HP, dmg);
      if (local_var(LVAR_Door_HP) <= 0) then begin // destroy door
          set_local_var(LVAR_Trapped, STATE_INACTIVE);
          set_local_var(LVAR_Locked, STATE_INACTIVE);
